...
export const validatePDFFile = async (
  ...
  try {
    ...
    const hasValidHeader = pdfSignature.every(
    ...
    if (!hasValidHeader) {
      return {
        ...
        suggestion:
          "Please select a different PDF file or try converting your document to PDF",
      };
    }
  } catch (error) {
    return {
 ...
 */
export const getPDFErrorMessage = (
  error: any,
  ...
  if (errorString.includes("InvalidPDFException")) {
    return {
      ...
      suggestion:
        "Please try a different PDF file or convert your document to PDF format.",
    };
  }
  ...
  if (errorString.includes("PasswordException")) {
    return {
      ...
      suggestion:
        "Please use a PDF without password protection or remove the password first.",
    };
  }

  if (
    errorString.includes("UnexpectedResponseException") ||
  ...
  if (errorString.includes("CORS") || errorString.includes("Access-Control")) {
    return {
      ...
      suggestion:
        "Please try uploading the PDF file directly instead of using a link.",
    };
  }
