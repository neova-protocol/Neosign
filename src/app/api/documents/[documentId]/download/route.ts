  ...
  try {
    ...
    if (!document || !document.fileUrl) {
      return NextResponse.json(
        { error: "Document not found or user not authorized" },
        { status: 404 },
      );
    }
    ...
    const originalPdfBytes = await fetch(fileUrl).then((res) =>
      res.arrayBuffer(),
    );
    ...
    console.log(
      "📝 Fields data received for PDF generation:",
      ...
      if (field.value && field.value.startsWith("data:image/png;base64,")) {
        try {
          ...
          if (page) {
            ...
            console.log(
              `📐 Page ${field.page} size: ${pageWidth} x ${pageHeight}`,
            );
            ...
            console.log(
              `📍 Drawing at position: x=${finalX}, y=${finalY}, w=${field.width}, h=${field.height}`,
            );
            ...
            console.log(
              `✅ Signature successfully drawn on page ${field.page}`,
            );
          } else {
        ...
        console.log(
          `⚠️ Field ${field.id} has non-PNG value:`,
    ...
    if (embeddingErrors.length > 0) {
      ...
      return NextResponse.json(
        { error: "Failed to embed some signatures.", details: embeddingErrors },
        { status: 500 },
      );
    }
    ...
    console.log(
      "✅ PDF saved successfully, size:",
      pdfBytesWithSignatures.length,
      "bytes",
    );
    ...
    console.error(
      `💥 Failed to generate signed PDF for document ${documentId}:`,
      error,
    );
    return NextResponse.json(
