...
const TimelineEvent: React.FC<{ event: DocumentEvent; isLast: boolean }> = ({
  event,
  isLast,
}) => (
  <div className="flex-1 flex flex-col items-center relative">
    ...
    <div className="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center border-2 border-white ring-1 ring-gray-300 z-10">
      {eventIcons[event.type as keyof typeof eventIcons] || <Clock className="h-5 w-5 text-gray-700" />}
    </div>
    <p className="text-sm font-medium text-gray-800 mt-2 text-center w-28">
      {getEventDescription(event)}
    </p>
    <p className="text-xs text-gray-500 mt-1">{event.userName}</p>
    {!isLast && (
      <div className="absolute top-8 left-1/2 w-full h-px bg-gray-300"></div>
    )}
  </div>
);

const SignatoryStatus: React.FC<{ signatory: Signatory; isFirst: boolean }> = ({
  signatory,
  isFirst,
}) => {
  ...
  return (
    <div
      className={`pt-3 ${isSigned && !isFirst ? "border-t-2 border-green-500" : ""} ${!isSigned && !isFirst ? "border-t" : ""}`}
    >
      <div className="flex items-center justify-between">
        <div>
          ...
          {isSigned && (
            <p className="text-sm text-gray-500">Valid proof of identity</p>
          )}
        </div>
        {isSigned ? (
...
export default function DocumentStatusPage() {
  ...
  const creationEvent = (document.events || []).find(
    (e) => e.type === "created",
  );
  const senderName = creationEvent?.userName || "Unknown";
  const timeAgo = creationEvent
    ? formatDistanceToNow(new Date(creationEvent.date))
    : "";
  ...
  return (
    <div className="bg-gray-100 min-h-screen p-4 sm:p-6 lg:p-8">
      <div className="max-w-7xl mx-auto bg-white rounded-xl shadow-sm p-8">
        ...
        <div className="flex justify-between items-start pb-8">
          <div className="flex items-center space-x-6">
            <div className="w-28 h-40 bg-gray-100 border rounded-md flex flex-col items-center justify-center p-2">
              ...
              <Button
                ...
                onClick={() =>
                  router.push(`/dashboard/sign/document/${documentId}`)
                }
              >
                See
              </Button>
          ...
          <div className="w-full max-w-md">
            ...
            <div className="space-y-2">
              {document.signatories.map((s, index) => (
                <SignatoryStatus
                  key={s.id}
                  signatory={s}
                  isFirst={index === 0}
        ...
        {/* Dotted Separator */}
        <div
          className="my-8 h-px bg-repeat-x bg-center"
          style={{
            backgroundImage:
              "url(\"data:image/svg+xml,%3csvg width='100%25' height='100%25' xmlns='http://www.w3.org/2000/svg'%3e%3crect width='100%25' height='100%25' fill='none' stroke='%23E5E7EB' stroke-width='4' stroke-dasharray='1%2c 8' stroke-linecap='round'/%3e%3c/svg%3e\")",
          }}
        ></div>
        ...
        <div className="flex pt-4">
          {(document.events || []).map((event, index) => (
            <TimelineEvent
              key={event.id}
              event={event}
              isLast={index === (document.events || []).length - 1}
            />
          ))}
        </div>
      </div>
