...
export default function TemplatesPage() {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const res = await fetch("/api/user-templates", { method: "POST", body: formData });
    if (res.ok) {
  ...
  const handleRenameUserTemplate = async (id: string, newName: string) => {
    ...
    if (res.ok) {
      setUserTemplates(prev => prev.map(t => t.id === id ? { ...t, name: newName } : t));
    }
    setRenamingId(null);
    setRenameValue("");
  };
  ...
  return (
    <div className="max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <Button onClick={handleUploadClick} className="bg-blue-500 hover:bg-blue-600 text-white px-6 py-2 rounded-lg flex items-center space-x-2" disabled={isUploading}>
      ...
      {userTemplates.length > 0 && (
        <div className="mb-12">
          <h3 className="text-lg font-semibold text-gray-900 mb-6 text-center">My Templates</h3>
                ...
                <CardContent className="p-6">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="mt-6 pt-4 border-t border-gray-200">
                        <div className="bg-blue-100 rounded px-3 py-2 text-center flex gap-2 justify-center">
                          <Button className="bg-blue-500 hover:bg-blue-600 text-white px-6 py-1 rounded text-sm" onClick={() => handleUserTemplateOpen(template)}>
                            Open
                          </Button>
                          <Button className="bg-gray-200 text-gray-700 px-4 py-1 rounded text-sm" variant="ghost" onClick={() => handleDeleteUserTemplate(template.id)}>
                            Delete
                          </Button>
                          <a href={template.fileUrl} download className="bg-gray-200 text-gray-700 px-4 py-1 rounded text-sm ml-2">Download</a>
                  ...
                  <div className="text-center">
                    {renamingId === template.id ? (
                      <form onSubmit={e => { e.preventDefault(); handleRenameUserTemplate(template.id, renameValue); }} className="flex gap-2 justify-center items-center">
                        <input value={renameValue} onChange={e => setRenameValue(e.target.value)} className="border rounded px-2 py-1 text-sm" />
                        <Button type="submit" size="sm" className="px-2 py-1 text-xs">Save</Button>
                        <Button type="button" size="sm" className="px-2 py-1 text-xs" variant="ghost" onClick={() => setRenamingId(null)}>Cancel</Button>
                      </form>
                    ) : (
